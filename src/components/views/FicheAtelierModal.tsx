...
export const FicheAtelierModal: React.FC<FicheAtelierModalProps> = ({ devis, onClose, onEditDevis }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/40 z-50 flex items-center justify-center p-3 sm:p-5 md:p-6 overflow-y-auto backdrop-blur-[3px] animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-6xl w-full max-h-[92vh] flex flex-col overflow-hidden border border-slate-200/90 ring-1 ring-black/5">
        
        {/* Top Header Bar (No Print) */}
        <div className="no-print px-5 sm:px-6 py-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 bg-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-500/30 shrink-0">
              <Scissors className="w-5 h-5" />
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="font-extrabold text-base sm:text-lg tracking-tight">Fiche Technique Atelier & Découpe</h2>
                <span className="text-xs bg-blue-500/20 text-blue-300 border border-blue-400/30 px-2.5 py-0.5 rounded-lg font-mono font-bold">
                  {currentDevis.numero}
                </span>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 active:scale-95 text-white text-xs font-bold px-3.5 sm:px-4 py-2 rounded-xl transition shadow-md shadow-blue-600/30 cursor-pointer"
            >
        ...
        <div className="no-print bg-slate-100/95 border-b border-slate-200/90 p-3 sm:px-5">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex flex-col justify-between text-left p-2.5 sm:p-3 rounded-xl sm:rounded-2xl transition-all cursor-pointer min-h-[58px] sm:min-h-[64px] border ${
                    isSel
                      ? 'bg-blue-600 text-white shadow-md shadow-blue-600/25 border-blue-500 ring-2 ring-blue-500/20'
                      : 'bg-white text-slate-700 hover:text-blue-700 hover:bg-blue-50/40 border-slate-200/90 shadow-2xs hover:border-blue-200'
                  }`}
                >
                  <div className="flex items-center justify-between gap-1.5 w-full">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`p-1.5 rounded-lg shrink-0 ${isSel ? 'bg-blue-500/40 text-white' : 'bg-slate-100 text-blue-600'}`}>
                        <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                      </div>
                      <span className="font-bold text-xs truncate">{t.label}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-2 pt-1 border-t border-slate-100/20 w-full text-[11px]">
                    <span className={`text-[10px] truncate ${isSel ? 'text-blue-100' : 'text-slate-400'}`}>
                      {t.sublabel}
                    </span>
                    <span className={`px-2 py-0.5 rounded-md font-mono font-bold text-[10px] shrink-0 ${
                      isSel 
                        ? 'bg-blue-700/80 text-white shadow-2xs' 
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-slate-50 printable-area space-y-4 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 [&::-webkit-scrollbar-track]:bg-slate-100/50">
          
          {/* Header Card */}
          <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs uppercase font-bold text-blue-600 tracking-wider">Atelier de Menuiserie Aluminium</span>
              </div>
              ...
              <p className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-3">
                <span>Dossier : <strong className="text-slate-900 font-mono">{currentDevis.numero}</strong></span>
                <span>•</span>
                <span>Client : <strong className="text-slate-900">{currentDevis.client_nom || 'Client Atelier'}</strong></span>
                <span>•</span>
                <span>Date : <strong className="text-slate-900 font-mono">{currentDevis.date}</strong></span>
              </p>
            </div>

            <div className="flex items-center gap-3 shrink-0 flex-wrap">
              <div className="bg-blue-50/90 border border-blue-200 px-4 py-2 rounded-2xl text-center">
                <p className="text-[10px] text-blue-700 uppercase font-bold tracking-wider">Barres 6m Profilés</p>
                <p className="text-xl font-black text-blue-900 font-mono mt-0.5">{result.totalProfileBarsCount} <span className="text-xs font-bold">barres</span></p>
              </div>
              <div className="bg-emerald-50/90 border border-emerald-200 px-4 py-2 rounded-2xl text-center">
                <p className="text-[10px] text-emerald-700 uppercase font-bold tracking-wider">Surface Totale Verre</p>
                <p className="text-xl font-black text-emerald-900 font-mono mt-0.5">{result.totalGlassAreaM2} <span className="text-xs font-bold">m²</span></p>
              </div>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {result.cuttingPieces.map((p, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/40 transition">
                      <td className="px-4 py-3 font-semibold text-slate-800">{p.elementLabel}</td>
                      <td className="px-4 py-3">
                        <span className="font-mono font-bold text-blue-700 bg-blue-50 border border-blue-200/60 px-2 py-0.5 rounded text-xs">
                          {p.profilRef}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-semibold text-slate-900">{p.profilDesignation}</td>
                      <td className="px-4 py-3 text-center">
                      ...
                      <td className="px-4 py-3 text-right font-mono font-black text-base text-slate-950 bg-slate-50/50">
                        {p.lengthCm.toFixed(1)} <span className="text-xs text-slate-500 font-normal">cm</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2.5 py-1 rounded-md text-xs font-black shadow-2xs ${
                          p.angleRight === '45°' 
                      ...
                      <td className="px-4 py-3 text-center font-mono font-black text-slate-950 text-base">
                        ×{p.quantity}
                      </td>
                  ...
                  <div className="flex items-start justify-between border-b border-slate-100 pb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-900 text-sm">{deb.profilDesignation}</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-1">
                        Métrage net total : <strong className="text-slate-800 font-mono">{deb.totalLinearMeters.toFixed(2)} m</strong>
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1 font-mono">Chute moy: {deb.scrapPercentageAverage.toFixed(1)}%</p>
                    </div>
                  </div>

                    {/* Bars visual cuts */}
                    <div className="space-y-3">
                      <p className="text-xs font-bold text-slate-700">Plan de coupe optimisé :</p>
                      {deb.allocatedBars.map((bar, bIdx) => {
                        ...
                        return (
                          <div key={bIdx} className="bg-slate-50 border border-slate-200/90 rounded-xl p-3.5 text-xs space-y-2.5">
                            <div className="flex justify-between items-center text-xs text-slate-700 font-semibold">
                              ...
                              <span className="font-mono text-slate-600 text-[11px] bg-white border border-slate-200 px-2.5 py-0.5 rounded-md">
                                Reste chute : <strong className="text-amber-700 font-bold">{bar.scrapCm.toFixed(1)} cm</strong>
                              </span>
                            </div>
                            ...
                            <div className="w-full bg-slate-200/80 h-7 sm:h-8 rounded-lg overflow-hidden flex border border-slate-300 shadow-inner">
                              {bar.cuts.map((c, cIdx) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`${colorClass} border-r border-white/60 flex items-center justify-center text-[11px] text-white font-mono font-black truncate px-1 shadow-2xs hover:brightness-110 transition-all`}
                                  >
                              ...
                              {bar.scrapCm > 0 && (
                                <div
                                  ...
                                  className="bg-amber-100/90 text-amber-800 text-[10px] flex items-center justify-center font-mono truncate font-bold border-dashed border-l border-amber-300 px-1"
                                >
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 pt-1">
                              {bar.cuts.map((c, cIdx) => {
                                ...
                                return (
                                  <span 
                                    key={cIdx}
                                    className="inline-flex items-center gap-1 bg-white border border-slate-200 text-slate-700 px-2 py-0.5 rounded-md font-mono text-[10px] font-semibold"
                                  >
                                    <span className={`w-1.5 h-1.5 rounded-full ${colorClass}`}></span>
                                    <span>{c.lengthCm.toFixed(1)} cm</span>
                                  </span>
                                );
                              })}
                              {bar.scrapCm > 0 && (
                                <span className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 text-amber-800 px-2 py-0.5 rounded-md font-mono text-[10px] font-bold">
          ...
          <div className={`space-y-6 ${activeTab !== 'composants' ? 'hidden print:block' : 'block'}`}>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono font-bold text-xs bg-blue-50 text-blue-800 border border-blue-200 px-3 py-1 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono font-bold text-xs bg-emerald-50 text-emerald-800 border border-emerald-200 px-3.5 py-1 rounded-xl">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                <table className="w-full text-left text-xs sm:text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {result.accessories.map((acc, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-5 py-3.5 font-mono text-blue-700 font-bold">{acc.reference || 'STD'}</td>
                  ...
                  <tfoot className="bg-slate-50/80 border-t border-slate-200 font-mono font-bold">
                    <tr>
                      <td colSpan={5} className="px-5 py-3.5 text-right text-slate-700 font-sans text-xs">
                        TOTAL ESTIMATION QUINCAILLERIE & JOINTS (HT) :
                      </td>
                      <td className="px-5 py-3.5 text-right text-emerald-700 font-black text-base">
                ...
                <tbody className="divide-y divide-slate-100 font-mono text-slate-800">
                  {result.glassItems.map((g, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-5 py-3.5 font-sans text-blue-700 font-bold">{g.vitrageType}</td>
                      <td className="px-5 py-3.5 text-right font-black text-slate-900 text-base">{g.hauteurCm.toFixed(1)} cm</td>
                      <td className="px-5 py-3.5 text-right font-black text-slate-900 text-base">{g.largeurCm.toFixed(1)} cm</td>
                      <td className="px-5 py-3.5 text-center font-black text-slate-900 text-base">×{g.quantity}</td>
                      <td className="px-5 py-3.5 text-right font-black text-emerald-700 text-base">{g.totalAreaM2.toFixed(3)} m²</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="text-xs text-slate-500 border-t border-slate-200 pt-4 flex flex-col sm:flex-row justify-between items-center gap-2">
