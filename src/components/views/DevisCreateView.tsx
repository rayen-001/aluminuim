...
const PROFILE_OPTION_NAMES: Record<string, string> = {
  ...
  'EX45 1212': 'EX45 1212 — Ouvrant porte fenêtre EX45',
  'EX45 1215': 'EX45 1215 — Ouvrant fenêtre porte-feuille EX45',
  'EX45 1218': 'EX45 1218 — Ouvrant fenêtre ligne droite EX45',
  'EX45 1312': 'EX45 1312 — Parclose droite F=15mm EX45',
  'Ex45 1310': 'Ex45 1310 — Parclose arrondie F=15mm EX45',
...
export const DevisCreateView: React.FC<DevisCreateViewProps> = ({
  ...
  const { clients, addClient, articlesMap, devisList, saveDevis, settings } = useApp();
  ...
  const applyTypeOptions = (idx: number, typeDef: ProductTypeDef, familyId: string) => {
    ...
    const isStore = typeDef.category === 'standalone_store' || familyId === '67';
    const isMousti = typeDef.category === 'standalone_mousti' || familyId === '68';
  ...
  const getDrawingSVG = (item: DevisItemState): string => {
    ...
    if (typeDef.category === 'standalone_store' || item.family_id === '67' || (item.include_menuiserie === false && item.store_enabled)) drawType = 'store';
    if (typeDef.category === 'standalone_mousti' || item.family_id === '68' || (item.include_menuiserie === false && item.mousti_enabled)) drawType = 'mousti';
  ...
  const handleCreateClient = (e: React.FormEvent) => {
    ...
    const created = addClient({
      ...
      matricule_fiscale: newClientMF.trim(),
      solde_creance: 0
    });
    ...
    <div className="p-3 sm:p-6 max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-semibold text-gray-700">Client</label>
                <button
                  type="button"
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="flex-1 border border-gray-300 rounded-xl px-3.5 py-2.5 text-sm bg-white focus:ring-2 focus:ring-blue-500 font-medium"
                >
                ...
                <button
                  ...
                  className="p-2.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl border border-blue-200 transition cursor-pointer shrink-0"
                >
                ...
                >
                  ...
                  {item.is_manual ? (
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Section Menuiserie Aluminium (Châssis) avec Contrôle d'inclusion */}
                        ...
                        <div className="pt-2 border-t border-gray-200/80 space-y-3">
                          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200 shadow-2xs">
                            <label className="flex items-center gap-2.5 text-xs font-bold text-slate-800 cursor-pointer select-none">
                              <input
                                type="checkbox"
                                checked={item.include_menuiserie !== false}
                                onChange={e => updateItem(index, { include_menuiserie: e.target.checked })}
                                className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 cursor-pointer"
                              />
                              <span className="flex items-center gap-1.5">
                                🪟 Inclure le châssis / cadre Menuiserie Aluminium (Fenêtre / Porte)
                              </span>
                            </label>
                            {item.include_menuiserie === false ? (
                              <span className="text-[11px] font-bold text-amber-800 bg-amber-100 border border-amber-300 px-2 py-0.5 rounded-md shadow-2xs">
                                ❌ Châssis exclu (0.000 DT)
                              </span>
                            ) : (
                              <span className="text-[11px] font-bold text-blue-800 bg-blue-100 border border-blue-300 px-2 py-0.5 rounded-md shadow-2xs">
                                ✓ Châssis inclus
                              </span>
                            )}
                          </div>
                          ...
                          {item.include_menuiserie === false ? (
                            <div className="p-3 bg-amber-50/80 border border-amber-200 rounded-xl text-xs text-amber-900 flex items-center gap-2 font-medium">
                              ...
                              <span>
                                Le coût du châssis aluminium (profilés, vitrage et accessoires fenêtre) est fixé à <strong>0.000 DT</strong>. Seuls les équipements configurés ci-dessous (Store Volet Roulant ou Moustiquaire) seront calculés et facturés dans le devis.
                              </span>
                            </div>
                          ) : (
                            <div className="space-y-3">
                              ...
                              {!isChassiFixe && !item.is_garde_corps && (
                                <div className="pt-2 border-t border-gray-100">
      ...
      {newClientModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 sm:p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-bold text-gray-900">Ajouter un Nouveau Client</h3>
                  <p className="text-[11px] text-gray-500">Sera automatiquement lié à ce devis</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateClient} className="space-y-3.5">
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">
                  Nom & Prénom / Société <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ex: Mohamed Ben Salah, Société XYZ..."
                    className="w-full border border-gray-300 rounded-xl pl-9 pr-3.5 py-2 text-sm focus:ring-2 focus:ring-blue-500 font-medium"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ex: 98 123 456 / 22 789 000"
                    className="w-full border border-gray-300 rounded-xl pl-9 pr-3.5 py-2 text-sm font-mono focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl pl-9 pr-3.5 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-xl pl-8 pr-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-xl pl-8 pr-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500 font-mono"
                    />
                  </div>
                </div>
              </div>
